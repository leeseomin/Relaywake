...
export class SurvivorScene extends Phaser.Scene {
  ...
  private readonly orbiters: OrbiterRuntime[] = [];
  ...
  private updateWeapons(): void {
    for (const state of this.abilities.owned()) {
      ...
      const stats = this.abilities.effectiveStats(state.id);
  ...
  private updateOrbiters(_delta: number): void {
    const state = this.abilities.get('axe');
    if (!state) {
      this.clearOrbiters();
      return;
    }
    const stats = this.abilities.effectiveStats('axe');
    const desiredCount = Math.max(1, stats.count);
    while (this.orbiters.length < desiredCount) {
      const visual = this.acquireSprite('weapon-sword').setScale(1.8).setDepth(105);
      this.orbiters.push({ visual, textureKey: 'weapon-sword', index: this.orbiters.length, lastHitAt: new Map() });
    }
    while (this.orbiters.length > desiredCount) {
      const orbiter = this.orbiters.pop();
      if (orbiter) this.releaseSprite(orbiter.textureKey, orbiter.visual);
    }
    ...
    for (let index = 0; index < this.orbiters.length; index += 1) {
      ...
      if (!orbiter) continue;
      orbiter.index = index;
      const angle = this.elapsedSeconds * stats.projectileSpeed + (index / this.orbiters.length) * Math.PI * 2;
      ...
      const candidates = this.enemyGrid.queryCircle(x, y, 36);
      for (const enemy of candidates) {
        const lastHit = orbiter.lastHitAt.get(enemy.id) ?? -999;
        if (!enemy.active || this.elapsedSeconds - lastHit < 0.42) continue;
  ...
  private clearOrbiters(): void {
    while (this.orbiters.length > 0) {
      const orbiter = this.orbiters.pop();
      if (orbiter) this.releaseSprite(orbiter.textureKey, orbiter.visual);
    }
  }
  ...
  private acquireSprite(textureKey: string): Phaser.GameObjects.Sprite {
    ...
    if (!pool) {
      pool = new ObjectPool<Phaser.GameObjects.Sprite>({
        ...
        activate: (sprite) => {
          sprite.setActive(true).setVisible(true).setAlpha(1).setScale(1).setRotation(0).setFlip(false, false).clearTint();
        },
        deactivate: (sprite) => {
          sprite.anims.stop();
          sprite.setActive(false).setVisible(false).setPosition(-10000, -10000).setAlpha(1).setRotation(0).setScale(1).clearTint();
        },
