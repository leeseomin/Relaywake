...
- 상단 선택 목록의 기존 4개 캐릭터를 `pixel-character-maker-mongle.html`에서 고정 생성한 로즈글라스 스카우트, 별꼬리 시프, 달토끼 워든, 모래뿔 브루저로 교체했습니다.
- 생성기의 24×24 A/B 걷기 프레임을 게임용 A/B/A/B 4프레임 스프라이트 시트로 구성했습니다.
- 기존 저장 데이터 호환성을 위해 내부 캐릭터 ID와 능력치는 유지하고, 표시 이름·설명·스프라이트만 교체했습니다.
- 파이어 마스터와 전용 화염핵, 화염 보너스는 변경하지 않았습니다.
